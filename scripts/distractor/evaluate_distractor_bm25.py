...
def _evaluate_once(
    ...
    docs = [(str(r[cfg.data.doc_id_column]), str(r[cfg.data.doc_text_column])) for r in corpus_rows]
    queries = {str(r[cfg.data.query_id_column]): str(r[cfg.data.query_text_column]) for r in query_rows}
...
@hydra.main(config_path="conf", config_name="eval_distractor_config")
def main(cfg: DictConfig) -> None:
    logger.info("Eval distractor config:\n%s", OmegaConf.to_yaml(cfg))
    ...
    logger.info("Saved distractor pipeline outputs to %s", Path.cwd())
