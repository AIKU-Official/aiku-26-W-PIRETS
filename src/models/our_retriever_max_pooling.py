...
class OurRetrieverMaxPooling(nn.Module, RetrievalModel):
    ...
    def _max_pooling(self, model_output, attention_mask):
        ...
        # 패딩 토큰은 아주 작은 값으로 보내서 Max에 안 잡히게 처리
    ...
    def encode(self, input_ids, attention_mask):
        ...
        # Pooling 방식 선택 (기본값: Mean Pooling)
        if self.config.get('use_pooler', False):
            embeddings = outputs.pooler_output
        else:
            # embeddings = self._mean_pooling(outputs, attention_mask)  # mean pooling
            embeddings = self._max_pooling(outputs, attention_mask)     # max pooling
